...
def run(sd, keys, cycles, out):
    cmd = [EMU, NEO, "--headless", "--storage", sd, "--cycles", str(cycles),
           "--type-keys", f"8000000:{keys}", "--screenshot", out]
...
@pytest.mark.parametrize("name,keys,cycles", [
    ("title", "", 7_000_000),             # écran de présentation
    ("holding", "FMMD", 60_000_000),      # français, mise 2, donne
    ("showdown", "FMMD5D", 90_000_000),   # change la 5e carte, abattage
    ("showdown_en", "EMMD5D", 90_000_000),  # même partie en anglais
])
